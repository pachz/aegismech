...
module.exports = async (req, res) => {
  ...
  if (!resendApiKey || !toEmail || !fromEmail) {
    return res.status(500).send('Server misconfiguration: missing env vars');
  }
  ...
  try {
    ...
    if (error) {
      return res.status(502).send('Email provider error.');
    }

    return res.status(200).send('Message Sent Successfully!');
  } catch (err) {
    return res.status(500).send('Server Error: failed to send email.');
  }
};
